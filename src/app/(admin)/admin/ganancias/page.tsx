...
export default async function GananciasPage({ searchParams }: Props) {
  ...
  const totales = byMonth.reduce(
    ...
    { ventasBrutas: 0, costoProduccion: 0, gananciaNeta: 0 }
  );
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Distribución mensual por socio</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
          <p className="text-xs text-gray-500 uppercase tracking-wide">Costo producción {selectedYear}</p>
          <p className="text-2xl font-bold text-gray-900 mt-1">{formatCurrency(totales.costoProduccion)}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
          <p className="text-xs text-gray-500 uppercase tracking-wide">Ganancia neta {selectedYear}</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-gray-50">
                ...
                <th className="text-right py-3 px-4 font-medium text-gray-600 border-l border-gray-200">Ganancia</th>
                {partners.map((p) => (
            ...
            <tbody className="divide-y divide-gray-50">
              {byMonth.map((m) => (
                <tr key={m.month} className={`hover:bg-gray-50/50 transition-colors ${m.numVentas === 0 ? "opacity-40" : ""}`}>
