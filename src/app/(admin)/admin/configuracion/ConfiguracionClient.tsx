...
export default function ConfiguracionClient({ partners, totalPct }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
        ...
        {remaining > 0 && (
          <p className="text-xs text-gray-400 mt-1">{remaining.toFixed(1)}% sin asignar (aparecerá en el reporte de ganancias)</p>
        )}
      </div>
                    ...
                    {editingId === p.id ? (
                      <>
                        <Button
                          ...
                          onClick={() => handleSaveEdit(p.id, p.order)}
                          disabled={loading}
                        >
                ...
                <td className="py-3 px-4 text-right">
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <Button
                      ...
                      size="icon"
                      className="h-8 w-8 text-gray-500"
                      onClick={() => { setShowNew(false); setNewName(""); setNewPct(""); }}
                    >
                      <X className="h-4 w-4" />
                    </Button>
                  </div>
